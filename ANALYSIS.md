...
> Based on the [Governance Health KPI Report v1.0](https://gov-health.intersectmbo.org/) (Intersect Civics Committee, 3 Dec 2025) and the yaci-store Parquet export schema.
...
## Part 2: Yaci-Store Data Schema

### Overview

Yaci-store exports its data as Parquet files with Hive-style partitioning into `data/analytics/main/`. Tables are partitioned by `date=YYYY-MM-DD` or `epoch=NNN` depending on the table. The schema mirrors the original PostgreSQL tables but with some differences:
- `block_time` columns are `TIMESTAMP WITH TIME ZONE` (not unix integers)
- JSON columns (`params`, `voting_stats`, `details`, `pool_owners`, `state`) are `VARCHAR`
- The `epoch` table is empty — epoch data is derived from `adapot` and `block`
- `local_treasury_withdrawal` is not present — derived from `gov_action_proposal` + `gov_action_proposal_status`

Key table groups:
- **Governance**: `gov_action_proposal`, `gov_action_proposal_status`, `voting_procedure`, `drep`, `drep_registration`, `drep_dist`, `delegation_vote`, `committee`, `committee_member`, `committee_registration`, `committee_state`, `constitution`, `gov_epoch_activity`
- **Epoch & Protocol**: `epoch_param`, `adapot`, `epoch_stake`
- **Pools**: `pool`, `pool_registration`, `pool_retirement`
- **Blocks & Transactions**: `block`, `transaction`
...
| details | varchar (JSON) | Structure varies by type (see below) |
| epoch | int | |
| slot | bigint | |
| block | bigint | |
...
| voting_stats | varchar (JSON) | Detailed vote breakdown (see below) |
| epoch | int | PK part 3 |
...
|--------|------|-------|
| epoch | int | PK |
| state | varchar (JSON) | JSON object with CC member states |
...
> **Note:** This table directory is empty in the yaci-store Parquet export. Epoch-level data is derived from `adapot` (financial), `block` (block counts, start/end times), and `transaction` (tx counts).
...
|--------|------|-------|
| epoch | int | PK |
| params | varchar (JSON) | Full protocol parameters |
| cost_model_hash | varchar(64) | |
| slot | bigint | |
| block_time | timestamp with time zone | |

**`params` JSON** contains all Cardano protocol parameters including governance thresholds (Conway era onwards):
- `drep_deposit`, `gov_action_deposit`, `drep_activity`, `gov_action_lifetime`
- `committee_min_size`, `committee_max_term_length`
- `drep_voting_thresholds` — object with keys like `dvt_motion_no_confidence`, `dvt_committee_normal`, `dvt_ppnetwork_group`, `dvt_ppgov_group`, etc. (each with `numerator`/`denominator`)
- `pool_voting_thresholds` — object with keys like `pvt_motion_no_confidence`, `pvt_committee_normal`, `pvt_ppsecurity_group`, etc. (each with `numerator`/`denominator`)
- Standard protocol params (`min_fee_a`, `min_fee_b`, `max_block_size`, etc.)

**Note:** Key naming differs from some documentation — `drep_voting_thresholds` not `d_rep_voting_thresholds`, and `pvt_ppsecurity_group` / `dvt_ppnetwork_group` (no underscore between `pp` and group name).
...
| pool_owners | varchar (JSON) | Array of owner key hashes |
| relays | varchar (JSON) | Array of relay objects |
| metadata_url | text | |
...
> **Note:** This table is not present in the yaci-store Parquet export. Treasury withdrawal data is derived from `gov_action_proposal` (type = `TREASURY_WITHDRAWALS_ACTION`) joined with `gov_action_proposal_status` (status = `ENACTED`).

---

#### `block`

Block data. Used for deriving per-epoch block counts and epoch start/end times (since the `epoch` table is empty).
...
## Part 4: Key JSON Columns Reference

These are the structured JSON columns (stored as VARCHAR in Parquet) most important for governance analytics. In DuckDB, use `json_extract_string(col, '$.key')` to access fields.

### 1. `gov_action_proposal_status.voting_stats`

This is the single most valuable column for many KPIs. It contains pre-computed vote tallies for each governance body (CC, DReps, SPOs) per action per epoch. The JSON is flat (not nested by governance body). Fields:
...
Simple JSON array of hex key hashes: `["abc123...", "def456..."]`. In DuckDB, parse with `CAST(pool_owners AS VARCHAR[])` + `unnest()`. Needed for SPO default stance computation (matching pool owner credentials to delegation_vote entries).
